...
| 用户需求关键词 | 图表类型 | 使用的形状 |
|--------------|----------|-----------|
| 流程、步骤、过程 | 流程图 | ellipse（开始/结束）+ rect（步骤）+ diamond（判断） |
| 架构、系统、模块 | 架构图 | rect（模块）+ hexagon（接口）+ cylinder（数据库） |
| 对比、比较、方案 | 对比分析图 | 使用 subgraph 并列展示 |
| 实验、算法、数据 | 实验流程图 | rect（步骤）+ diamond（判断）+ ellipse（数据） |
| 多层、分层、层次 | 多页图表 | 使用 page 分页 |
