 ...
 *
 * 兼容性：
 * - 保留 window.* facade（KaTeXCache / katexCache / renderKatexCached / getKatexCacheStats）
 * - 提供 ESM 显式导出
 */
...
export class KaTeXCache {
  ...
  render(tex, options = {}) {
    ...
    // 性能监控
    if (this.enableMonitoring && typeof window !== 'undefined' && window.PerfMonitor) {
...
export let katexCache = (typeof window !== 'undefined' && window.katexCache) ? window.katexCache : null;

export function getKaTeXCache() {
  if (katexCache) return katexCache;
  if (typeof window !== 'undefined' && window.katexCache) return window.katexCache;
  return null;
}

/**
 * 初始化并挂载到 window（浏览器环境）
 * @param {Object} [options]
 * @returns {KaTeXCache|null}
 */
export function initKaTeXCacheGlobal(options = {}) {
  ...
  katexCache = window.katexCache;

  // 提供全局便捷方法
  window.renderKatexCached = function(tex, renderOptions) {
    return window.katexCache.render(tex, renderOptions);
  };
  ...
  return window.katexCache;
}

/**
 * ESM 侧便捷渲染：仅浏览器环境可用
 */
export function renderKatexCached(tex, options) {
  if (typeof window === 'undefined' || !window.katexCache) {
    throw new Error('katexCache is not initialized');
  }
...
export function getKatexCacheStats() {
  if (typeof window === 'undefined' || !window.katexCache) return null;
  return window.katexCache.getStats();
}

// 向后兼容：浏览器环境下自动初始化
if (typeof window !== 'undefined') {
  initKaTeXCacheGlobal();
