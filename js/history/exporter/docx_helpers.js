 ...
 */
export function dedupeRepeatedSequence(input) {
 ...
 * @returns {string} 去重后的字符串
 */
export function dedupeSplitRepeats(input) {
    if (!input || typeof input !== 'string') return input;
    let s = input;
 ...
 */
export function sanitizeCardLine(text) {
    if (!text) return '';
 ...
 */
export function truncateForCard(input, maxLength) {
    if (!input) return '';
 ...
 */
export function selectCardTitle(translationMarkdown, ocrMarkdown, fallbackName) {
 ...
 */
export function getOcrMarkdown(data) {
    if (!data) return '';
 ...
 */
export function getTranslationMarkdown(data) {
    if (!data) return '';
 ...
 */
export function collectIntroCardInfo(payload, helpers = {}) {
...
// 兼容层：保留原 window.PBXDocxHelpers
if (typeof window !== 'undefined') {
  window.PBXDocxHelpers = window.PBXDocxHelpers || {};
  Object.assign(window.PBXDocxHelpers, PBXDocxHelpers);
}
