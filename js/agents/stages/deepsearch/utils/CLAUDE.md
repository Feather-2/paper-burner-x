...
| 文件 | 职责 |
|------|------|
| `stage-api.js` | stageApi 规范与工厂封装（create/validate/extract/merge/child），复用 shared/utils 的实现；补充 DeepSearch 侧 StageApi 类型（含可选 modelRouter/aiApiService/eventBus/runTool 等注入点） |
...
- ModelPriceEntry / 默认价格表：用于预算估算的 per-1K token 价格配置，可由 userConfig 覆盖（注意合并时需要防 prototype pollution）。
