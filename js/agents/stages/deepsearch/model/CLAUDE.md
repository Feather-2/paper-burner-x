...
| `budget.js` | 维护 `state.L2.budgetState` 并触发预算事件 |
...
| 预算事件 | `deepsearch.budget.warning/exceeded`，`reasons` 为 `tokens/cost`，payload 含 `budget/total/ratios`（`total.estimatedCostUSD` 为估算成本） |
