...
| `checkpoint.js` | CheckpointManager + snapshot 生成/迁移；`CHECKPOINT_SCHEMA_VERSION`（当前为 `"1.0"`）标识 schema；`normalizeCheckpointStrategy()` 归一化策略输入（默认 LITE）；loadCheckpoint 兼容旧 schema 并补齐关键字段（如 todos/L2 flags 等）；`cloneValue()` 安全拷贝 |
...
| Backtrack 结果 | `BacktrackResult` 返回 `success/reason/state`；失败时提供 `error`；可在 `details` 中携带诊断字段用于 UI/日志/二次决策 |
