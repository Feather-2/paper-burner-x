...
- 分析门槛支持 `state.globalConfig.analysisGates[mode]` 覆盖默认阈值（quick/wider/deeper）。
...
- **引用溯源**：`get-source` 通过 `state.L0.sources`/`context.sourceManager` 列出可用源文档，并支持 `start`/`maxLength` 截取原文（默认 5000、最大 10000）
