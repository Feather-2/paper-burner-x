...
- 查询/等待任务：`getTaskStatus(taskId)` 或 `waitForTask(taskId, timeout)`（供 get-task-result 使用）。
- 调整并发与清理：配置 `DEEPSEARCH_MAX_RUNNING_TASKS`、`DEEPSEARCH_TASK_TTL_MS`、`DEEPSEARCH_TASK_CLEANUP_INTERVAL_MS`、`DEEPSEARCH_TASK_RESULT_PREVIEW_CHARS`。
