...
import { protoSafeReviver, injectSystemHint, extractJsonCandidate } from "../../shared/index.js";
import { createLogger } from "../../shared/utils/logger.js";

const logger = createLogger("agents");

/** @private Maximum allowed input text length (2MB) to prevent memory exhaustion. */
export const MAX_INPUT_TEXT_LEN = 2 * 1024 * 1024;
/** @private Maximum allowed sources count. */
export const MAX_SOURCES_COUNT = 100;

export function toRawText(input) {
...
export function resolveStageTraceContext(api) {
...
export function resolveErrorBoundary(api) {
...
export function ensureTraceContext(api, traceContext) {
  try {
...
export function buildFallbackPackage({ rawText, input, runContext, defaultChunkOptions }) {
  // Best-effort heuristic pipeline: avoids LLM calls, still satisfies hard gates.
  const normalized = normalizeText(rawText);
  const chunkOptions = { ...defaultChunkOptions, ...(toChunkOptions(input) || {}) };
...
export async function runNormalizeStep(rawText, traceContext, api, emit) {
  api.checkCancelled();
...
export async function runChunkStep({ normalized, input, defaultChunkOptions, traceContext, api, emit }) {
  api.checkCancelled();
  const chunkOptions = { ...defaultChunkOptions, ...(toChunkOptions(input) || {}) };
  ...
  return { chunks, chunkOptions };
}

export async function runSlidePlanStep({ chunks, runContext, traceContext, api, emit, tracedAiApiService }) {
  api.checkCancelled();
...
export async function runClaimsStep({ chunks, slideIntents, normalized, traceContext, api, emit }) {
  api.checkCancelled();
...
export async function runAlignStep({ slideIntents, claims, runContext, traceContext, api, emit, tracedAiApiService }) {
  api.checkCancelled();
  ...
  return alignedSlides;
}

export async function runBuildPackageStep({ runContext, normalized, alignedSlides, claims, evidenceLedger, traceContext, api }) {
  api.checkCancelled();
  const sources = [
    {
      sourceId: "user_text",
  ...
  return await traceContext.withSpan("textprep.build.contentPackage", async (span) => {
