...
| `generating-phase.js` | 生成页面 HTML、QA 校验、降级 fallback、style lock 建立、图片规划（可选预算/策略 + 成本估算）、取消/超时控制、step 执行钩子（start/finishExecution） |
...
- **Image Planning**: `ImagePlanner.plan` 产出 imageSlots，并估算成本/待生成列表；在存在 `imagePolicy` 或 `imageBudget` 时启用预算/策略相关决策（含 slot 成本估算，依赖 `DESIGN_PHASE_DEFAULTS.costPer*Slot`）。
...
- 将阶段接入统一执行器时，给 `runGeneratingPhase` 传入 `startExecution/finishExecution`，用于 step 级别 trace/上下文管理。
