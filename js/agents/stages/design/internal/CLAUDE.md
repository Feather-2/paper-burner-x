...
| `deck-operations.js` | VisualHandler 封装、watchdog 管理、deck 更新事件 |
...
- 保存版本/回滚：`saveVersion()` + `backtrackTo()` / `backtrackToLastCheckpoint()`。
- 断点续跑：`resumeDesignAgentLoop(checkpointId)` + `createResumeToolExecutor()`。
