...
export class ResourceLock {
  ...
  async acquire(resourceId) {
    ...
    while (true) {
      try {
        ...
        logger.debug('Lock acquired', { resourceId, lockPath });
        ...
        return {
          ...
          release: async () => {
            try {
              ...
              logger.debug('Lock released', { resourceId, lockPath });
            } catch (err) {
              logger.warn('Lock release failed', { resourceId, error: err?.message });
            }
          },
        };
      } catch (err) {
        if (err.code !== 'EEXIST') {
          throw new Error(`Failed to acquire lock for ${resourceId}: ${err.message}`);
        }
        ...
        try {
          ...
          if (age > this.staleTimeoutMs) {
            // 锁已过期，尝试清理
            logger.warn('Stale lock detected, attempting cleanup', { resourceId, age });
            try {
              fs.unlinkSync(lockPath);
              continue; // 重试获取锁
            } catch (cleanupErr) {
              // 清理失败，可能被其他进程抢先
              logger.debug('Stale lock cleanup failed', { resourceId, error: cleanupErr?.message });
            }
          }
        } catch (statErr) {
          // 锁文件可能已被删除，重试
        ...
        if (Date.now() - startTime > this.acquireTimeoutMs) {
          throw new Error(`Lock acquire timeout for ${resourceId} after ${this.acquireTimeoutMs}ms`);
        }
  ...
  async tryAcquire(resourceId) {
    ...
    try {
      ...
      logger.debug('Lock acquired (non-blocking)', { resourceId, lockPath });
      ...
      throw new Error(`Failed to try acquire lock for ${resourceId}: ${err.message}`);
    }
  }
