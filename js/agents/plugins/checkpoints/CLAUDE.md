...
`plugins/checkpoints` 负责把一次运行的消息、工具调用、结果等快照写入存储，并维护索引以支持按“最新 / 指定 ID / 指定步数”恢复。
...
- **runId**: 运行分区标识。写入前会做安全化处理（仅保留 `a-zA-Z0-9._-`，其他字符替换为 `_`），空值回退为 `run`；显式拒绝 `.`/`..`（路径遍历）并抛错。目录为 `.agents/runs/<runId>/checkpoints`。
...
- **JSON 大小上限**: 读取 `index.json` 与检查点 JSON 时分别限制最大字符数（默认 `2_000_000` / `5_000_000`），超出将拒绝解析以避免内存/DoS 风险。
...
- **索引锁**: `withIndexLock` 保证同一 `runId` 的索引读写在单进程内串行化（不提供跨进程/跨标签页锁）。
