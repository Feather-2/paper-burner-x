...
- **SilentErrorReporter**: 收集错误样本，支持 `enabled`、`maxSamples`、`onError` 回调；提供 `report()`（context: `location`/`category`/`operation`）、`getStats()`、`export()`、`getRecent()`、`clear()`、`size`、`setEnabled()` / `isEnabled()`。
- **ErrorCategory**: `RECOVERABLE` / `DEGRADED` / `CRITICAL` 三类，用于分类统计。
- **ErrorEntry**: 记录 `message`、截断后的 `stack`、`location`、`category`、`operation`、`ts`。
