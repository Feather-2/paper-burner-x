/**
 * Content-quality evaluator (backward compatible).
 *
 * This file preserves the original EvaluateStage API while also providing a
 * grader wrapper (`contentGrader`) that can be used inside the new eval harness.
 *
 * @module eval/graders/content
 */
...
/**
 * Wrapper grader to use EvaluateStage inside EvalHarness.
 *
 * Options:
 * - stage: EvaluateStageOptions (optional)
 * - input: Partial<EvaluationInput> (optional; merged with { content: output })
 *
 * @type {{ type: 'content', grade: (output: unknown, config: GraderConfig) => Promise<GraderResult> }}
 */
export const contentGrader = {
  type: "content",
  async grade(output, config) {
    const options = (config && typeof config === "object" && config.options && typeof config.options === "object")
      ? config.options
      : {};

    const stageOptions = options.stage && typeof options.stage === "object" ? options.stage : {};
    const stage = new EvaluateStage(stageOptions);

    const input = options.input && typeof options.input === "object" ? options.input : {};
    const content = typeof output === "string" ? output : output == null ? "" : String(output);

    const result = await stage.run(null, { ...input, content });
    return {
      graderType: "content",
      passed: result.passed,
      score: Number.isFinite(result.score) ? result.score : 0,
      reason: result.passed ? "Content quality passed" : "Content quality failed",
