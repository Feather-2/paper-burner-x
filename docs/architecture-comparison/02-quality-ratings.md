...
```
  ...
  5 ★★★★★  ┃████████████████████████████████████████┃  18 模块
           ...
           ┃ runtime/hooks/ core/crdt/ core/contracts/┃
           ┃ core/di/ core/archive/ core/node-compat/ ┃
           ┃ core/sandbox/ core/webruntime/            ┃
           ┃ stages/textprep/                          ┃
           ┃                                           ┃
  4 ★★★★☆  ┃████████████████████████████████████████┃  13 模块
           ┃ plugins/ (12 production) runtime/tools/   ┃
           ┃ sdk/ prompts/ stages/deepsearch/ skills/  ┃
           ┃ stages/codesearch/ shared/ testing/       ┃
           ┃ runtime/core/ stages/design/              ┃
           ┃ core/kernel+buses/                        ┃
           ┃                                           ┃
  3 ★★★☆☆  ┃                                            0 模块
           ┃ （全部已升级到 4★）                        ┃
...
|--------|------:|------:|:------:|---------------|--------|
| core/kernel+buses | ~10 | ~4200 | ★★★★ | 干净的总线设计，Lamport Clock 事件序 | 已修复：Archive 持久化防抖；背压策略简化 |
...
| runtime/core | ~6 | ~3300 | ★★★★ | Template Method 模式骨架 | 已修复：Mixin→组合；Resolver→DI；Hook→Middleware 统一 |
| runtime/tools | 15 | 3178 | ★★★★ | Worker 隔离执行 + 内置工具集 | 无 |
| runtime/safety | 4 | 1140 | ★★★★★ | Fork bomb 6 变体检测 + 17 敏感路径拦截 | 无 |
| runtime/hooks | 5 | 1484 | ★★★★★ | 20+ token 净化规则 + 3 种 Hook 类型 | 无 |
| sdk | 17 | 3709 | ★★★★ | dispose guard、injection scanner 等安全机制 | 部分死代码 |
| plugins (12) | ~30 | ~7500 | ★★★★ | 12 个生产级插件全部可用 | 3 个空插件目录 |
| stages/deepsearch | 66 | ~15000 | ★★★★ | 完整 3 阶段循环 + 16 工具 | 部分大文件 |
| stages/design | 64 | ~19600 | ★★★★ | 7 阶段流水线 + Style Lock 机制 | 已修复：blackboard 已拆分为 DesignState + DesignCheckpoints + Facade |
...
| skills | 9 | ~1200 | ★★★★ | 3 级加载 (repo > user > system) + TTL 缓存 | 无 |
| prompts | 12 | ~1600 | ★★★★ | 模板引擎 + 格式化器 | 无 |
| storage | 7 | ~1800 | ★★★★★ | IndexedDB CRUD + 配额管理 | 无 |
| eval | 8 | ~1500 | ★★★★★ | 多评分器框架 | 无 |
| shared | ~35 | ~8000 | ★★★★ | 全面的工具函数集 | 无 |
| testing | 2 | ~600 | ★★★★ | Mock 套件 + 场景运行器 | 无 |

## agentsdk-go 对比

Go SDK 约 60 个文件，整体均匀分布在 **4 stars**。没有模块低于 4 星，但也没有模块达到 5 星——这与 Go 版本范围较小、功能子集有限有关。JS 版本在 Phase 1-4 完成后已收敛到全模块 4 星及以上，同时顶部仍有 18 个模块保持 5 星生产级水准。
