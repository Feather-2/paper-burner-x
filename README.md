...
- **多文件、快速并发翻译引擎**：能够支持上传多文件，能快速翻译长论文（如在短时间20s内完成一篇)，并支持自定义文件处理/翻译任务并发数
- **自定义翻译提示词**：可以自定义prompt,以支持客制化需求。
- **自定义模型设置**：改用Base URL，可自动通过（v1/models）API以自动检测可用模型。
- **历史记录面板**：能够通过IndexDB来存储历史的转换记录，并支持多种模式（原文译文/对比模式）来查看OCR与翻译后的版本，支持基础的公式、图片和表格渲染
